...
from .model import XCapFusionIntermediate, ConvAggregator
...
def predict(dataset):
    ...
    real_captions = []
    FOLDER_NAMES=[]
    predictor = IntermediateLabelPredictor(config_path)
    ...
    for _, entry in tqdm(dataset_raw.iterrows(), desc="Processing dataset", unit="entry", total=len(dataset_raw)):
        ...
        soft_prompt = f"The image show: {', '.join(label)}."
        ...
        if len(files) == 0:
            ...
            try:
                ...
                if not os.path.isfile(filename):
                        continue
                try:
                    predicted_caption = predictor.predict(filename,soft_prompt)
    ...
    output_csv_file =  os.path.join(config["paths"]["predictions_path"],"predictions.csv")
...
class IntermediateLabelPredictor:
    def __init__(self, config_path):
        ...
        self.model = XCapFusionIntermediate(self.prefix_length, clip_length=prefix_length_clip, prefix_size=prefix_dim,
                                num_layers=num_layers)
        self.model.load_state_dict(torch.load(config["paths"]["checkpoints_load"], map_location=self.device))
        self.model = self.model.eval().to(self.device)


    def predict(self, image_path, soft_prompt):
        ...
        with torch.no_grad():
            ...
            tokens_label = torch.tensor(self.tokenizer.encode(soft_prompt)).to(self.device)
            max_seq_len_labels=50
            padding1=max_seq_len_labels - tokens_label.shape[0]
            if padding1 > 0:
                tokens_label = torch.cat((tokens_label, torch.zeros(padding1, dtype=torch.int64, device=self.device)))
            elif padding1 < 0:
                tokens_label = tokens_label[:max_seq_len_labels]
            embedding_label = self.model.gpt.transformer.wte(tokens_label)
            embedding_label=embedding_label.unsqueeze(0)
            ...
            prefix_embed=self.model.crossAtt(embedding_label, prefix_projections) # batch x 10 x 768
