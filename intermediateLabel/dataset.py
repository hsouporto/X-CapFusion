...
class MIMICDataset(Dataset):
    def __init__(self, group_data='train', config_path="config.yaml"):
        ...
        for _, entry in tqdm(self.dataset_raw.iterrows(), desc="Processing dataset", unit="entry", total=len(self.dataset_raw)):
            ...
            tokens_label_tensor = torch.tensor(self.tokenizer.encode(soft_prompt))
            self.label_tokens.append(tokens_label_tensor)
        ...
        self.max_seq_len = min(int(all_len_captions.mean() + all_len_captions.std() * 10), int(all_len_captions.max()))
        all_len_labels = torch.tensor([len(self.label_tokens[i]) for i in range(len(self))]).float()
        self.max_seq_len_labels = min(int(all_len_labels.mean() + all_len_labels.std() * 10), int(all_len_labels.max()))
    ...
    def pad_tokens(self, item: int):
        ...
        tokens_label = self.label_tokens[item]        
        padding1= self.max_seq_len_labels - tokens_label.shape[0]
        if padding1 > 0:
            tokens_label = torch.cat((tokens_label, torch.zeros(padding1, dtype=torch.int64)))
        elif padding1 < 0:
            tokens_label = tokens_label[:self.max_seq_len_labels]
        mask = tokens_caption.ge(0) 
        tokens_caption[~mask] = 0
        mask = mask.float()   
        mask = torch.cat((torch.ones(self.prefix_length), mask), dim=0)
        return tokens_caption, tokens_label,  mask
    ...
    def __getitem__(self, idx):
        tokens_caption_tensor,tokens_label_tensor, mask = self.pad_tokens(idx)
        ...
        return image_tensor,  tokens_caption_tensor, tokens_label_tensor, mask
