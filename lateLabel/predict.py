...
from .model import XCapFusionLate, ConvAggregator
...
def predict(dataset):
    ...
    real_captions = []
    FOLDER_NAMES=[]
    predictor = LateLabelPredictor(config_path)
        ...
        if len(files) == 0:
            ...
            try:
                ...
                if not os.path.isfile(filename):
                        continue
                try:
                    predicted_caption = predictor.predict(filename)
...
class LateLabelPredictor:
    def __init__(self, config_path):
        """Inicializa o modelo e carrega os pesos necessários."""
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        repo = "microsoft/rad-dino"
        self.clip_model = AutoModel.from_pretrained(repo).to(self.device)
        self.preprocess = AutoImageProcessor.from_pretrained(repo)
        ...
        self.model = XCapFusionLate(self.prefix_length, clip_length=prefix_length_clip, prefix_size=prefix_dim,
    ...
    def predict(self, image_path):
        ...
        with torch.no_grad():
            ...
            prefix_embed = self.model.project(conv_output).view(-1, self.prefix_length, self.model.gpt_embedding_size)
...
def generate_beam(
    ...
    config = load_config("./lateLabel/config.yaml")
    beam_size=config["generation"]["beam_size"]
    entry_length=config["generation"]["entry_length"]
    temperature=config["generation"]["temperature"]
    prompt=config["generation"]["prompt"]
    model.eval()
    stop_token_index = torch.tensor([198, 198], device=next(model.parameters()).device)  # Lista de stop tokens
    tokens = None
    scores = None
