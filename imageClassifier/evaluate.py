...
def evaluate(dataset):
    ...
    data = pd.read_csv(os.path.join(config["output"]["predictions"], f"predictions_{dataset}"))
    output=os.path.join(config["output"]["metrics"], f"metrics_{dataset}")
    y_true = [list(map(int, label.split())) for label in data["True Label"]] 
    y_pred = [list(map(int, label.split())) for label in data["Predicted Label"]] 
