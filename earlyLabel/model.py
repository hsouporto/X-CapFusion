...
from utils.modules import LabelSoftPrompt, ConvAggregator, TransformerMapper, CrossAttentionLayer
...
class XCapFusionEarly(nn.Module):
    def __init__(self, prefix_length: int, clip_length: Optional[int] = None, prefix_size: int = 768,
        ...
        super(XCapFusionEarly, self).__init__()
        self.prefix_length = prefix_length
        self.mapppingLabel = LabelSoftPrompt(14, 768, 50)
        ...
        for param in self.conv.parameters():
            param.requires_grad = True
        for param in self.mapppingLabel.parameters():
            param.requires_grad = True
        for param in self.mapppingLabel.parameters():
            param.requires_grad = True
    ...
    def forward(self, image_tensor: torch.Tensor, tokens_caption_tensor: torch.Tensor, labels_one_hot: torch.Tensor,
        ...
        embedding_label_extended = self.mapppingLabel(labels_one_hot.float())
        ...
        prefix_projections=self.crossAtt(embedding_label_extended, prefix_projections) 
